              ...
              <div class="chat-messages-area custom-scrollbar" ref="chatScroll">
                ...
                <div v-for="msg in chatMessages" :key="msg.id" class="message-row" :class="msg.direction === 'OUTBOUND' ? 'msg-out' : 'msg-in'">
                  <div class="message-bubble" :class="msg.direction === 'OUTBOUND' ? 'bubble-out' : 'bubble-in'">
                    ...
                    <!-- KODI MPYA: RENDER CONTENT INAYOTAMBUA MEDIA NA KUONYESHA TEXT KWA UBORA -->
...
// ==========================================
// 🔥 MEDIA PARSING LOGIC ILIYOBORESHWA ZAIDI (SAIZI NA MWONEKANO KAMA WHATSAPP)
// ==========================================
const renderMessageContent = (text) => {
  if (!text) return '';

  let safeText = text.replace(/https:\/\/api\.kedeshlimited\.com/g, 'https://apibulksms.kedeshlimited.com');

  // Angalia Picha
  if (safeText.startsWith('[MEDIA:IMAGE]')) {
    const url = safeText.replace('[MEDIA:IMAGE]', '').trim();
    return `<div class="media-container image-type">
              <img src="${url}" class="chat-image" alt="Picha" loading="lazy" onerror="this.onerror=null; this.src='/logo/image.png';" />
            </div>`;
  }
  // Angalia Video
  else if (safeText.startsWith('[MEDIA:VIDEO]')) {
    const url = safeText.replace('[MEDIA:VIDEO]', '').trim();
    return `<div class="media-container video-type"><video src="${url}" controls class="chat-video" preload="metadata"></video></div>`;
  }
  // Angalia Audio/Voice Notes
  else if (safeText.startsWith('[MEDIA:AUDIO]')) {
    const url = safeText.replace('[MEDIA:AUDIO]', '').trim();
    return `<div class="media-container audio-type"><audio src="${url}" controls class="chat-audio"></audio></div>`;
  }
  // Angalia Documents
  else if (safeText.startsWith('[MEDIA:DOCUMENT]')) {
    const url = safeText.replace('[MEDIA:DOCUMENT]', '').trim();
    return `<a href="${url}" target="_blank" class="document-link"><span class="doc-icon">📄</span> <span class="doc-text" style="color: #111b21 !important;">Pakua Faili</span></a>`;
  }

  // Kama ni meseji ya kawaida au template (Maandishi)
  const urlRegex = /(https?:\/\/[^\s]+)/g;
  const formattedText = safeText.replace(urlRegex, '<a href="$1" target="_blank" class="chat-link">$1</a>');

  return `<div class="msg-text-body" style="color: #111b21 !important;">${formattedText}</div>`;
};
...
/* ==========================================
   🔥 CSS YA VIPIMO VYA CHAT BUBBLES
   ========================================== */
.message-row {
  display: flex;
  width: 100%;
...
/* 🔥 MAANDISHI KUONEKANA WAZI KWA WINO MWEUSI */
.msg-text-body {
  color: #111b21 !important;
  line-height: 1.4;
  padding-bottom: 16px;
...
/* 🔥 KUDHIBITI UKUBWA WA PICHA NA VIDEO (KAMA WHATSAPP) */
.media-container {
  position: relative;
  margin-bottom: 16px;
  border-radius: 8px;
  overflow: hidden;
  max-width: 240px; /* Reduced to make it compact like WhatsApp */
}
...
.msg-meta {
  ...
  align-items: center;
  gap: 4px;
  background: rgba(255,255,255,0.85); /* Kiboksi kyeupe chepesi ili kionekane vizuri juu ya picha */
  padding: 2px 6px;
  border-radius: 10px;
